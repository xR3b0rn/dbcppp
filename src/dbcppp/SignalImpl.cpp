...
double template_decode8(const Signal* sig, const void* _8byte) noexcept
{
    ...
    {
        unsigned int sign     = (data & 0x80000000); //01111111 10000000 00000000 00000000
        unsigned int exponent = (data & 0x7F800000); //01111111 10000000 00000000 00000000
        unsigned int mantessa = (data & 0x007FFFFF); //00000000 01111111 11111111 11111111
        float normalized_exponent = static_cast<float>((static_cast<signed char>((exponent>>23) - 127)));
        float normalized_mantessa = static_cast<float>(1) + static_cast<float>(mantessa) / pow(static_cast<float>(2), static_cast<float>(23));
        float result = normalized_mantessa * pow(static_cast<float>(2),static_cast<float>(normalized_exponent));
        if (sign != 0) result = result * static_cast<float>(-1);
        if (fabs(result) < pow(static_cast<float>(10), static_cast<float>(-38))) result = 0;
